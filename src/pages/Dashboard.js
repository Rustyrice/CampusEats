...
export default function Dashboard() {
  ...
  const handleDeleteMeal = async (id) => {
    try {
      ...
      await ingredientService.deleteRatingForDish(id)
  ...
  const getFilteredMeals = (searchQuery, meals) => {
    ...
    return meals.filter((meal) =>
      meal.name.toLowerCase().includes(lowerCaseSearchQuery),
    )
  }
        ...
        <div className="w-full bg-white-100 py-20">
          <div className="flex justify-between items-center mb-4 ml-4 mr-4">
            ...
            <button
              ...
              className="bg-green-400 hover:bg-green-700 text-white py-2 px-4 ml-2 rounded-lg"
            >
              Search
            </button>
          ...
          <PopUp trigger={addButton} onClose={() => setAddButton(false)}>
            <div className="w-96 h-96 overflow-y-auto">
              <form onSubmit={handleSubmit}>
                ...
                <button
                  type="submit"
                  className="bg-green-400 hover:bg-green-700 text-white py-2 px-4 rounded-lg mt-2"
                >
                  {' '}
                  Add Meal{' '}
                </button>
